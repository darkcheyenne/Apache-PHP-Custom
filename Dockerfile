...
&& apt-get install -y php-curl ftp zip unzip php7.2-zip curl php7.2-common s3cmd php7.2-gd php-xml php-xml brotli memcached php-memcached nasm apache2 libapache2-mod-php7.2 php-fpm php7.2-mbstring php7.2-mysql wget \
...
&& cat /etc/php/7.2/apache2/php.ini | grep memory_limit && cat /etc/php/7.2/apache2/php.ini | grep upload_max_filesize && cat /etc/php/7.2/apache2/php.ini | grep post_max_size \
&& a2enconf log
